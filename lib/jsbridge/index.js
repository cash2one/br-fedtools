! function(win) {
	...
	var BRG_Core = {
		call: function(model, method, params, success, failure, timeoutSecond) {
			var timeout;
			timeout = timeoutSecond > 0 ? setTimeout(function() {
				BRG_Core.onFailure(timeout, {
					ret: "TIMEOUT"
				})
			}, timeoutSecond) : BRG_Private.getSid();
	...
	var BRG_Private = {
		...
		unregisterCall: function(timeout) {
			...
			return delete callbackMap[successP], delete callbackMap[failureP], obj
		},
		getSid: function() {
		...
		callMethodByPrompt: function(model, method, paramString, timeout) {
			var url = LOCAL_PROTOCOL + "://" + model + ":" + timeout + "/" + method + "?" + paramString,
				uValue = WV_PROTOCOL + ":";
			this.params[PARAM_PREFIX + timeout] = paramString, window.prompt(url, uValue);
			console.log(url);
		},
